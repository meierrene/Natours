...
const displayMap = locations => {
  mapboxgl.accessToken = 'pk.eyJ1IjoibWVpZXJyZW5lIiwiYSI6ImNsYTMzM3ZvYzBpYjgzbnJwZWxhanVnZzYifQ.o9h8s8-Jr1NwqzZDmaav7Q';
  const map = new mapboxgl.Map({
    container: 'map',
    // container ID
    // style: 'mapbox://styles/meierrene/cla3ytc4x000a14o08ledsdzv', // Black and white map
    style: 'mapbox://styles/meierrene/cla32iano001114rtq8xm6xsr',
    // Colorful map
    scrollZoom: false,
...
const login = async (email, password) => {
  try {
    const res = await (0, _axios.default)('http://127.0.0.1:3000/api/v1/users/login', {
      method: 'POST',
      data: {
        email,
        password
...
const logout = async () => {
  try {
    const res = await (0, _axios.default)('http://127.0.0.1:3000/api/v1/users/logout', {
      method: 'GET'
    });
...
const signup = async (name, email, password, passwordConfirm) => {
  try {
    const res = await (0, _axios.default)('http://127.0.0.1:3000/api/v1/users/signup', {
      method: 'POST',
      data: {
        name,
        email,
    ...
    if (res.data.status === 'success') {
      (0, _alerts.showAlert)('success', 'Your new has created with success!');
      window.setTimeout(() => {
        location.assign('/');
      }, 1500);
    }
  } catch (err) {
    (0, _alerts.showAlert)('error', 'Error to create your account! Please try again.');
  }
};
...
const bookTour = async tourId => {
  const stripe = Stripe('pk_test_51M3YalLXlN7RtnXzZUpbfocSXWf5K15A700OVM2o0eWD0nX6iu1qlNpt5izuW9EjmV6eq4Kc45Fa2K4yAznqPgyZ00jcr4cDjX');
  try {
...
if (signupForm) signupForm.addEventListener('submit', e => {
  e.preventDefault();
  document.querySelector('.btn--green').textContent = 'Signing up...';
